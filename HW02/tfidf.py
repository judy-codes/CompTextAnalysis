...
class TfIdf:
    ...
    def train_seen(self, word: str, count: int=1):
        ...
        # Add your code here!
        if word in self._fdist:
            self._fdist[lower(word)] += count
        else:
            self._fdist[lower(word)] = count
    ...
    def inv_docfreq(self, word: int) -> float:
        ...
        """
        #num total docs / # of docs w the term
        count = 0
        for doc in self._docs:
            if word in doc:
                count += 1
    ...
    def vocab_lookup(self, word: str) -> int:
        ...
        if word in self._fdist:
            word = lower(word)
            if self._fdist[word] >= self._unk_cutoff:
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--test_dataset", help="Dataset for test",
                           ...
                           required=False)
    argparser.add_argument("--test_another", help="Dataset for test",
                           type=str, default='0101.txt',
                           required=False)
    ...
    with open(os.path.join(args.root_dir, args.train_dataset)) as infile:
        ...
        # d = FreqDist(TreebankWordTokenizer().tokenize(str(data)))
    ...
    with open(os.path.join(args.root_dir, args.test_dataset)) as infile:
        data = infile.read()
        vector = vocab.doc_tfidf(data)
        for word, tfidf in sorted(vector.items(), key=lambda kv: kv[1], reverse=True)[:50]:
            print("%s:%i\t%f" % (word[1], word[0], tfidf))
    
    with open(os.path.join(args.root_dir, args.test_another)) as infile:
        data = infile.read()
        vector = vocab.doc_tfidf(data)
        for word, tfidf in sorted(vector.items(), key=lambda kv: kv[1], reverse=True)[:50]:
            print("%s:%i\t%f" % (word[1], word[0], tfidf))