...
class Sampler:
    ...
    def change_topic(self, doc, index, new_topic):
        ...
        # terms to topics, documents to topics, and the specific assignments
    ...
    def sample_probs(self, doc_id, index):
        ...
        for doc in self._doc_tokens:
            for word in self._doc_tokens[doc]:
                if word == term:
                    term_count += 1
        ...
        for kk in range(self._num_topics):
            ...
            word = self._doc_tokens[doc_id][index]
            p_word_topic = float(self._topics._topic_term[kk][word] + self._topics._default_beta)/float((sum(self._topics._topic_term[kk].values()) + len(self._vocab)))
            p_topic_doc = float(self._doc_counts[doc_id][kk] + self._alpha[kk]) / float(sum(self._doc_counts[doc_id].values()) + self._num_topics)
            sample_probs[kk] = p_word_topic * p_topic_doc

#Closes on so far, uses my understanding of alpha and beta 
            first = float(self._topics.word_in_topic(kk, word) + self._topics._default_beta)
            sec = float((sum(self._topics._topic_term[kk].values()) + len(self._vocab) * self._topics._default_beta))
            p_word_topic = first/sec
            #p_topic_doc = (doc_topic[i, i] + alpha) / (np.sum(doc_topic[i, :]) + num_topics * alpha)
            p_topic_doc = float(self._doc_counts[doc_id][kk] + self._alpha[kk]) / float(sum(self._doc_counts[doc_id].values()) + self._num_topics * self._alpha[kk])
            sample_probs[kk] = p_word_topic * p_topic_doc

#Revisiting 
            # first = float(len(self._topics._topic_term[kk]) + self._topics._beta)
            # sec = float(len(self._vocab)) #not len of vocab
            # p_word_topic = first/sec
            # #p_topic_doc = (doc_topic[i, i] + alpha) / (np.sum(doc_topic[i, :]) + num_topics * alpha)
            # p_topic_doc = float(self._doc_counts[doc_id][kk] + self._alpha[kk]) / float(len(self._doc_assign[doc_id]))
            # sample_probs[kk] = p_word_topic * p_topic_doc

#Doesn't work off by 1, theory provided by yen
            # first = float(self._topics._topic_term[kk][index] + self._topics._default_beta)
            # sec = term_count
            # p_word_topic = first/sec
            # #p_topic_doc = (doc_topic[i, i] + alpha) / (np.sum(doc_topic[i, :]) + num_topics * alpha)
            # p_topic_doc = float(self._doc_counts[doc_id][kk] + self._alpha[kk]) / float(sum(self._doc_counts[doc_id].values()) - len(self._vocab))
            # sample_probs[kk] = p_word_topic * p_topic_doc

#Used normalizer, not sure if I should be, also used my own beta
            # sample_probs[kk] = (self._topics.get_normalizer(kk)/(topic_sum + self._num_topics * beta))* ((self._topics.word_in_topic(kk, term)+self._num_topics)*self._alpha[kk])
            # topic_prob =self._topics.get_normalizer(kk)/((topic_sum + word_sum)* beta)
            # doc_prob = self._topics.word_in_topic(kk, term)
            # sample_probs[kk] = topic_prob * doc_prob
            # topic_term_prob = self._topics.get_observations(kk, term) / int(self._doc_counts.get(kk))
            # doc_prob = int(self._doc_counts.get(kk))/self._doc_counts.N()
            # sample_probs[kk] = topic_term_prob * doc_prob
