...
class Sampler:
    ...
    def sample_probs(self, doc_id, index):
        ...
        for kk in range(self._num_topics):
            p_word_topic = float(self._topics._topic_term[kk][term] + self._topics._default_beta)/float((sum(self._topics._topic_term[kk].values()) + len(self._vocab)))
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--num_iterations", help="Number of iterations",
                           type=int, default=1000, required=False)   
    #my code
    argparser.add_argument("--delete_duplicates", action="store_true", help="Delete duplicate documents",
                            required=False)   
    argparser.add_argument("--lda_wiki", help="wiki",
                           type=str, default='data/wiki',
                           required=False)
    ...
    search_path = u"%s/*.txt" % args.lda_wiki
