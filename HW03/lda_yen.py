...
class LdaTopicCounts:
    ...
    although words can have an asymmetric prior.
    """
...
class Sampler:
    ...
    def change_topic(self, doc, index, new_topic):
        ...
        # TODO: Add code here to keep track of the counts and assignments
        if old_topic != -1:
            assert new_topic == -1     
    ...
    def sample_probs(self, doc_id, index):
        ...
        Takes in a document and an index
        We are trying to look for a specific token
        Given a token, we want to determine the probability of topic i conditioned on the specific token 
        P(topic|token)
        return not just for topic i, but for all topics
        Distribution of topics in the document - What topics do we see in this document
        --> P(topic|token) x P(topic|document) for a specific topic
        P(topic|token) = C (count of type that was assigned topic i) + beta/ C (count of type in corpus)
        P(topic|document) = C (count of topic i in this document) + alpha / C (the count of each topic in the document - how many words you have in the document because every word is assigned a topic) --> alpha and beta will come in here
        ...
        sample_probs = {}
        tokens, assignments, counts = self.get_doc(doc_id)
        instance = tokens[index]

        # loop through doc_tokens to get 2: C (count of type in corpus)
        count_type_in_corpus = 0
        for eachDoc in self._doc_tokens:
            for eachToken in self._doc_tokens[eachDoc]:
                if eachToken == instance:
                    count_type_in_corpus +=1
         
        for eachTopic in range(self._num_topics):
            alpha = self._alpha[eachTopic]
            beta = self.beta
            p_topic_token = (beta + self._topics.get_observations(eachTopic, instance)) / count_type_in_corpus
            p_topic_document = (alpha + counts[eachTopic]) / len(tokens)
            sample_probs[eachTopic] = p_topic_token * p_topic_document
        return sample_probs
